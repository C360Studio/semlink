...
`go.mod` has `replace github.com/c360studio/semstreams => ../semstreams`. **A sibling
`../semstreams` checkout is required to build or test anything.** The full Docker demo
also needs a sibling `../semconnect`. Override locations with `SEMSTREAMS_ROOT` /
`SEMCONNECT_ROOT`. In Docker the semstreams tree is passed via the build's
`additional_contexts: semstreams` (see `Dockerfile` / `compose.semlink.yml`).
